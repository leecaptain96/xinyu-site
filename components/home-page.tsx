...
function usePortfolioMotion(root: React.RefObject<HTMLDivElement | null>) {
  useLayoutEffect(() => {
    ...
    const desktopMotion = window.matchMedia("(min-width: 769px) and (hover: hover)").matches;
    ...
    const context = gsap.context(() => {
      ...
      intro
        ...
        .fromTo("[data-hero-visual] img", { scale: 1.18, opacity: .74 }, { scale: 1, opacity: 1, duration: 2.1 }, "-=1.28")
      ...
      sections.forEach((section, index) => {
        ...
        section.querySelectorAll<HTMLImageElement>("img").forEach((image) => {
          ...
          if (desktopMotion) gsap.fromTo(image, { yPercent: 3 }, { yPercent: -3, ease: "none", scrollTrigger: { trigger: image, start: "top bottom", end: "bottom top", scrub: 1.2 } });
        });
      });
...
function localMediaSrc(src?: string) {
...
function GlobalAtmosphere() {
  ...
  useEffect(() => {
    ...
    return () => { cancelAnimationFrame(frame); window.removeEventListener("pointermove", follow); };
...
function Header() {
  ...
  useEffect(() => {
    let frame = 0;
    const update = () => { setScrolled(window.scrollY > 20); frame = 0; };
      ...
      <motion.div data-opening-stage initial={false} animate={{ opacity: 1, y: 0 }} transition={{ duration: .9, ease }} className="hero-editorial relative overflow-hidden rounded-[2rem] bg-[#17221c] text-white shadow-[0_35px_90px_rgba(28,36,31,.16)] md:rounded-[2.5rem]">
        ...
        <div className="grid min-h-[700px] md:grid-cols-[1.08fr_.92fr]">
          ...
          <motion.div data-hero-visual initial={false} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: .12, ease }} className="relative min-h-[580px] overflow-hidden md:min-h-full"><HeroConnections/>
            <Image src="/images/shi-xinyu-hero.webp" alt="石新雨商务形象照片" fill priority className="object-cover object-top" sizes="(max-width:768px) 100vw,46vw"/>
